...
public class UserController {
    ...
    @Operation(summary = "Get all users by gender", description = "This endpoint is used to retrieve all users by gender.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Users found successfully",
                    content = {@Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = UserResponse.class)))}),
    ...
    public ResponseEntity<List<UserResponse>> getAllUsersByGender(@PathVariable("gender") Gender gender) {
        List<UserResponse> users = userService.getAllUsersByGender(gender);
        return ResponseEntity.ok(users);
    }
}